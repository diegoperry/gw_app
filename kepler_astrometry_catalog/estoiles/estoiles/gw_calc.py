...
class GWcalc:
    ...
    def __init__(self,Mc,q,f0,R,inc,psi,sourceCoord,detCoord,
        ...
        assert u.get_physical_type(f0.unit) == u.get_physical_type('frequency')
        ...
        self.q = self.coordtransform(detCoord,sourceCoord)
        self.phip = phip
        self.phic = phic
        self.setup()
    ...
    def setup(self):
        ...
        self.s2 = np.sin(self.inc)**2
        self.s4 = np.sin(self.inc)**4
        self.c1 = np.cos(self.inc)
        self.c2 = np.cos(self.inc)**2
        self.c4 = np.cos(self.inc)**4
        self.c6 = np.cos(self.inc)**6
    ...
    def H(self,sourceCoord,psi_):
        ...
        Hp = 0.5*np.array(np.matmul(P,P.T) - np.matmul(Q,Q.T))
        Hc = 0.5*np.array(np.matmul(P,Q.T) + np.matmul(Q,P.T))
        return Hp,Hc
    ...
    # Multiply some M x N matrix by an B-long list of scalars, producing an (B, M, N) sized output
    @staticmethod
    def multiply_matrix_by_scalar_arr(M, arr):
        return (M.flatten() * arr[:, np.newaxis]).reshape(len(arr), *M.shape)
    ...
    def calc_h(self,theta_,phi_=None,A=None):
        ...
        if np.all(t_ < self.tc):
            ...
            if self.Ap.isscalar:
                term1 = self.Ap * self.Hp# * np.exp(self.phip.to(u.rad).value*1j)
                term2 = self.Ac * self.Hc# * np.exp(self.phic.to(u.rad).value*1j)
            else:
                term1 = self.multiply_matrix_by_scalar_arr(self.Hp, self.Ap)# * np.exp(self.phip.to(u.rad).value*1j))
                term2 = self.multiply_matrix_by_scalar_arr(self.Hc, self.Ac)# * np.exp(self.phic.to(u.rad).value*1j))
            h = (term1 + term2).real
        else:
            raise RuntimeError('For some times in input, binary reached coalescence.')
            self.Ap = 0
            self.Ac = 0
            h = 0.
        return h
    ...
    def coordtransform(detCoord,sourceCoord):
        ...
        Keyword arguments:
        detCoord -- z-axis of the telescope frame, SkyCoord object
        sourceCoord -- GW source coordinate, SkyCoord object. Must be in the same frame as detCoord.

        Returns:
        projection of sourceCoord in the telescope's frame, in cartesian representation
        '''
        # We can't use is_equivalent_frame here since transforming from a SkyOffsetFrame introduces
        # some new attributes, and is_equivalent_frame is only true if all attributes are exactly equivalent.
        assert sourceCoord.frame.name == detCoord.frame.name
    ...
    @staticmethod
    def invcoordtransform(detCoord,sourceCoordCart):
        '''
        Convert sourceCoordCart from cartesian coordinates back into a lat lon-like coordinate
        in the same frame as detCoord.
        ...
        Keyword arguments:
        detCoord -- z-axis of the telescope frame, SkyCoord object
        sourceCoordCart -- array of shape (3,) or (3, N) with GW cartesian coordinates.

        Returns:
        SkyCoord array of shape (N,) in the same frame as detCoord.
        '''
        # Handle (3,) input for a single vector
        if sourceCoordCart.ndim == 1:
            if sourceCoordCart.shape[0] != 3:
                raise ValueError("If sourceCoordCart is 1D, it must have length 3")
            sourceCoordCart = sourceCoordCart.reshape(3, 1)

        # Handle (3, N) batch case
        elif sourceCoordCart.ndim == 2:
            if sourceCoordCart.shape[0] != 3:
                raise ValueError("sourceCoordCart must have shape (3,) or (3, N)")
        else:
            raise ValueError("sourceCoordCart must have shape (3,) or (3, N)")
        ...
        TrMat = np.array([[c(alpha),-s(alpha)*c(beta),s(beta)*s(alpha)],
            [s(alpha),c(alpha)*c(beta),-s(beta)*c(alpha)],
            [0,s(beta),c(beta)]])
        # dnC = np.matmul(TrMat, sourceCoordCart)
        # sourceCoord = SkyCoord(u=dnC[0], v=dnC[1], w=dnC[2], representation_type='cartesian', frame=detCoord)
        dnC_all = TrMat @ sourceCoordCart   # shape (3, N)
        from astropy.coordinates import CartesianRepresentation

        cart = CartesianRepresentation(dnC_all[0] * u.one,
                                    dnC_all[1] * u.one,
                                    dnC_all[2] * u.one)

        sourceCoord = SkyCoord(cart, frame=detCoord)
        sourceCoord.representation_type = 'unitspherical'
        return sourceCoord